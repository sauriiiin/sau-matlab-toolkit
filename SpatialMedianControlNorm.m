...
%
% Modified by Saurin Parikh, April, 2018
% To be used with Control Normalization implementation in the 
% sau-matlab-toolkit.
% Modification: If nanmedian = 0, median will be determined by the non-zero
...
classdef SpatialMedianControlNorm < Closure
    properties
        windowsize
        windowshape
        windowfun
    ...
    methods
        function this = SpatialMedianControlNorm(varargin)
            this = this@Closure();
            ...
            if ~this.acceptzeros
                ...
                if ~isempty(iszero)
                    fit(iszero) = blockfun( fil(colsizes, @(x) x<=0), ...
                        this.window, @this.abs_median, 'positions', iszero);
                end
            end
        end
...
%         end
        ...
        function val = abs_median(~, x)
            % If the median was zero, I want the median of the non-zero
            % values
